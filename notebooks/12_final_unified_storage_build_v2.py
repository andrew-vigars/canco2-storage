"""Build the v2 unified Canadian geological CO2 storage GeoPackage.
...
Primary output
--------------
``data/processed/unified_storage/20260917_13_CanadaGeologicalStorageUnified_AV_v2.gpkg``
...
UNIFIED_SUBMISSION_FILENAME = (
    f"{UNIFIED_DATE.replace('-', '')}_13_"
    f"{UNIFIED_DATA_TYPE}_AV_{UNIFIED_BUILD_VARIANT}.gpkg"
)
...
def find_project_root(start: Path | None = None) -> Path:
    ...
    start : pathlib.Path or None, optional
        Starting directory for the upward search. If omitted, the current working
        directory is used.
...
def build_paths(
    ...
    source_files = {
        "AER": (
            source_dir
            / "aer_agreements"
            / "20260917_13_AERCarbonSequestrationAgreements_AV.gpkg"
        ),
        "ATLANTIC": (
            source_dir
            / "gsc_atlantic"
            / "20260917_13_AtlanticStorageCOS_AV.gpkg"
        ),
        "BC": (
            source_dir
            / "gbc_ne_atlas"
            / "20260917_13_BCStorageAtlas_AV.gpkg"
        ),
        "NATCARB": (
            source_dir
            / "natcarb_doe"
            / "20260917_13_NATCARBStorage_AV.gpkg"
        ),
    }
...
def validate_input_paths(paths: BuildPaths) -> None:
    ...
    if missing:
        ...
        raise FileNotFoundError(f"Required Notebook 12 input(s) missing:\n{joined}")
...
def validate_canonical_tables(tables: UnifiedTables) -> None:
    ...
    for name, gdf in {
        ...
        if gdf.crs is None or gdf.crs.to_epsg() != 3978:
            raise ValueError(f"{name} must use EPSG:3978.")
...
def _register_attribute_table(
    conn: sqlite3.Connection,
    table_name: str,
    description: str,
) -> None:
    """Register a nonspatial canonical table in ``gpkg_contents``."""
    conn.execute(
        """
...
def export_unified_tables(tables: UnifiedTables, output_path: Path) -> None:
    ...
    with closing(sqlite3.connect(output_path)) as conn:
        ...
        _register_attribute_table(
            conn,
            "storage_units",
            "Logical geological CO2 storage units used by the unified Canadian storage model.",
        )
        _register_attribute_table(
            conn,
            "storage_assessments",
            "Capacity, prospectivity, and reservoir assessment records linked to storage units and spatial features.",
        )
...
def validate_existing_gpkg(output_path: Path) -> dict[str, int]:
    ...
    with closing(sqlite3.connect(output_path)) as conn:
        ...
        # GeoPackage discovery: canonical nonspatial tables are registered as
        # attribute tables alongside the two spatial feature tables.
        gpkg_content_types = dict(
            conn.execute(
                "SELECT table_name, data_type FROM gpkg_contents "
                "WHERE table_name IN "
                "('storage_units', 'storage_features', "
                " 'storage_assessments', 'administrative_features');"
            ).fetchall()
        )
        ...
        if gpkg_content_types != expected_content_types:
            raise ValueError(
                "Canonical gpkg_contents registrations are incomplete or invalid: "
                f"{gpkg_content_types}"
            )
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Build or validate the Notebook 12 v2 unified Canadian geological storage GeoPackage."
        )
    )
    parser.add_argument(
        "--project-root",
...
def main() -> None:
    ...
    if args.validate_existing is not None:
        ...
        print("Notebook 12 v2 canonical GeoPackage validation passed.")
    ...
    print("Notebook 12 v2 unified storage build")
    print("---------------------------------")
