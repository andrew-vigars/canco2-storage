"""Build the final unified Canadian geological CO2 storage GeoPackage.
...
Primary output
--------------
``data/processed/unified_storage/20260917_13_CanadaGeologicalStorageUnified_AV.gpkg``
...
def export_unified_tables(tables: UnifiedTables, output_path: Path) -> None:
    ...
    with closing(sqlite3.connect(output_path)) as conn:
        tables.storage_units.to_sql(
            "storage_units", conn, if_exists="replace", index=False
        )
        tables.storage_assessments.to_sql(
            "storage_assessments", conn, if_exists="replace", index=False
        )
        conn.commit()
...
def build_unified_metadata(source_catalog: pd.DataFrame) -> pd.DataFrame:
    ...
    expected_filename = (
        ...
        f"{lookup['data_type']}_{lookup['creator_initials']}.gpkg"
    )
    if lookup["submission_filename"] != expected_filename:
        raise ValueError("Unified submission filename contract failed.")
    return metadata
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Build or validate the Notebook 12 unified Canadian geological storage GeoPackage."
        )
    )
    parser.add_argument(
        "--project-root",
...
def main() -> None:
    ...
    if args.validate_existing is not None:
        ...
        print("Notebook 12 canonical GeoPackage validation passed.")
    ...
    print("Notebook 12 unified storage build")
    print("---------------------------------")
